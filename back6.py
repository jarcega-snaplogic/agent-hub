...
if st.session_state.selected_session:
    ...
    # Use session state directly as the default value for multiselect
    selected_roles = st.multiselect(
        "Filter by Role",
        ["system", "user", "assistant", "tool"],
        default=st.session_state.filter_roles,
        key="filter_roles",
        on_change=None  # Remove explicit state update since Streamlit handles this automatically
    )
    ...
    if show_graph:
        # Apply centered container style only around the graph
        st.markdown(
            """
            <style>
                .graph-container {
