...
if show_graph:
    with st.container():
        ...
        selected_roles_graph = st.multiselect(
            "Filter by Role",
            ["system", "user", "assistant", "tool"],
            default=["system", "user", "assistant", "tool"],
            key="filter_roles_graph"  # Unique key
        )
...
# Use st.form to manage filtering
with st.form("filter_form"):
    selected_roles_history = st.multiselect(
        "Filter by Role",
        ["system", "user", "assistant", "tool"],
        default=["system", "user", "assistant", "tool"],
        key="filter_roles_history"  # Unique key
    )
...
if submit_button:
    if st.session_state.selected_session:
        history = fetch_history(st.session_state.selected_session)
        selected_roles_lower = [role.lower() for role in selected_roles_history]
        ...
        st.info("Please select a session to view execution history.")
